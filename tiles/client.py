...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the tiles Live Runner demo (capacity showcase).")
    ...
    parser.add_argument("--grid", type=int, default=3, help="split into GRID x GRID tiles (default 3 = 9 tiles)")
    parser.add_argument("--discovery", default=DEFAULT_DISCOVERY)
    parser.add_argument("--signer", default="", help="Remote signer base URL (on-chain/paid path).")
    parser.add_argument("--reserve-timeout", type=float, default=120.0, help="Max seconds to wait for a capacity slot per tile.")
    parser.add_argument("--dump-tiles", action="store_true", help="Write each input + processed part to ./tiles-parts/ (gitignored) to inspect the split.")
    return parser.parse_args()


def _split(img: np.ndarray, grid: int) -> Iterator[tuple[int, int, int, int, np.ndarray]]:
    # np.array_split handles sizes that don't divide evenly; yields (row, col, y0, x0, tile).
    y0 = 0
...
async def _reserve_with_retry(*, discovery_url: str, signer_url: str | None, deadline: float):
    # A full runner refuses the reserve; wait for a slot instead of failing the tile.
    delay = 0.25
    while True:
        try:
            return await reserve_session(discovery_url=discovery_url, app=APP_ID, signer_url=signer_url)  # Livepeer: 1
        except (NoRunnerAvailableError, NoOrchestratorAvailableError):
            if time.monotonic() >= deadline:
                raise
            await asyncio.sleep(delay + random.uniform(0, 0.1))  # jittered backoff avoids a thundering herd
...
async def _process_tile(
    *,
    ...
    session = None
    try:
        session = await _reserve_with_retry(discovery_url=discovery_url, signer_url=signer_url, deadline=deadline)
        ...
        out = cv2.imdecode(np.frombuffer(base64.b64decode(out_b64), dtype=np.uint8), cv2.IMREAD_COLOR)
        h, w = tile.shape[:2]
        if out.shape[:2] != (h, w):
...
async def main() -> None:
    ...
    log.info("split %s into %d tiles (%dx%d grid); fanning out one session per tile", input_path.name, len(pieces), grid, grid)
