...
Livepeer integration (grep `# Livepeer:`):
  1. register_runner()     — announce the app (with its capacity) to the orchestrator (startup)
  2. registration.close()  — deregister (cleanup)

/tile is an ordinary HTTP handler; its CPU work is offloaded to a thread so tiles run in parallel.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Live Runner tiles app demo (capacity showcase).")
    ...
    parser.add_argument("--capacity", type=int, default=4, help="Max concurrent sessions the orchestrator routes here (the capacity knob).")
    parser.add_argument("--work", type=int, default=4, help="Supersample factor: stylize at (work x width) x (work x height). Higher = heavier CPU per tile (raise if tiles finish too fast to see the capacity effect).")
...
def _stylize(img: np.ndarray, work: int) -> np.ndarray:
    # Supersample: stylize once at `work`x resolution, then downsample. CPU cost grows
    # with area (~work^2), so higher `work` makes each tile heavier — enough that raising
    # capacity (more tiles at once) is a visible wall-clock win. A single pass at higher
    # resolution stays anti-aliased and detailed, avoiding the mush of repeated passes.
    h, w = img.shape[:2]
    scale = max(1, work)
...
async def _handle_tile(request: web.Request) -> web.Response:
    ...
    out = await loop.run_in_executor(None, _process, base64.b64decode(b64), request.app["work"])
...
def main() -> None:
    ...
    async def _on_startup(app: web.Application) -> None:
        app["registration"] = await register_runner(  # Livepeer: 1
            ...
            app=APP_ID,
            mode="persistent",  # single-shot by nature; persistent until single-shot payment lands (go-livepeer#3955)
