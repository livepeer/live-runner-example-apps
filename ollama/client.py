...
Ordinary `openai` code with a fixed base_url and api_key. It has no idea it's
talking to Livepeer. The local gateway (gateway.py) sits at base_url, maps the
`model` to its runner app, and does discovery + payment.

    docker compose up -d                              # the network side
    uv run gateway.py &                                # the local gateway on :8080
    uv run client.py --model qwen2.5:0.5b --prompt "Hello!"
    uv run client.py --model llama3.2:1b  --prompt "Hello!"   # same gateway, other model
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chat with an Ollama Live Runner using the stock OpenAI library.")
    ...
    parser.add_argument("--model", default="qwen2.5:0.5b", help="An Ollama model from runners.json (e.g. llama3.2:1b).")
